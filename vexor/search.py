...
from .providers.local import MLXEmbeddingBackend, LocalEmbeddingBackend
...
class VexorSearcher:
    ...
    def _create_backend(self) -> EmbeddingBackend:
        ...
        if self.provider == "local":
            if self.local_device == "mlx":
                self._device = f"{self.model_name} via MLX (Metal GPU)"
                return MLXEmbeddingBackend(
                    model_name=self.model_name,
                    chunk_size=self.batch_size,
                    concurrency=self.embed_concurrency,
                )
