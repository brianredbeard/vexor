...
def check_api_connectivity(
    ...
    if normalized == "local":
        try:
            from ..providers.local import LocalEmbeddingBackend, MLXEmbeddingBackend, is_mlx_available
            ...
            if local_device == "cuda":
                ...
                if not is_mlx_available():
                    return DoctorCheckResult(
                        name="Local Model",
                        passed=False,
                        message="MLX backend not available",
                        detail="MLX requires macOS on Apple Silicon. Install with: uv pip install 'vexor[local-mlx]'",
                    )

            if local_device == "mlx":
                backend = MLXEmbeddingBackend(model_name=model)
            else:
