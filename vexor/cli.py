...
def local(
    ...
    device: str = typer.Option(
        ...
        "--device",
        "-d",
        help="Device for local inference: auto, cpu, cuda, mlx",
    ),
    ...
    if effective_device == "cuda":
        ...
        if "CUDAExecutionProvider" not in providers:
            ...
            raise typer.Exit(code=1)
    elif effective_device == "mlx":
        from .providers.local import is_mlx_available
        if not is_mlx_available():
            console.print(
                _styled("MLX not available (requires macOS on Apple Silicon). Install with: uv pip install 'vexor[local-mlx]'", Styles.ERROR)
            )
            raise typer.Exit(code=1)
    ...
    try:
        if effective_device == "mlx":
            from .providers.local import MLXEmbeddingBackend
            backend = MLXEmbeddingBackend(model_name=clean_model)
        else:
